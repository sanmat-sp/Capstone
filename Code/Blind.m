...
data=table(doc_no,fgrprt_2,result,'VariableNames',{'img_id' 'img' 'block_no'});
sqlwrite(conn,"capstone.blind",data)
...
elseif(length(BW_fgrprt) < length(bst_blk))
    Sh_diag(1:length(BW_fgrprt), :) = Sw_diag(1:length(BW_fgrprt), :);
end
...
params = [0; 3; 3; 0.001; 0; 0; 0.8; 0; 0; 0; 1.5; 10; 0];
...
for j=1:length(attacks)
    ...
    numParticles = 30; %30
    numIterations = 50; %50
    minParam=0.0001; %0.0001
    maxParam=0.001;  %0.001
    paramBounds=[maxParam,minParam];
    ...
    sgtitle('Watermarked Cover Image (Blind method)')
    ...
    title(attack)
    xlabel(['PSNR: '+string(PSNR)]);
    sgtitle('Extracted Watermark Image (Blind method)')

    if mse<=0.005
        Au=[Au,"Images match"];
    else
        Au=[Au,"Images do not match"];
    end
end
...
title('PSNR-CI and PSNR-EI against Attacks - Blind Method');
xlabel('Attacks');
ylabel('PSNR (dB)');
